  ...
  def launch
    ...
    if params[:roles] == "Learner"
      ...
      if @user.grades.where(enrollment_id: @enrollment.id, assignment_id: @assignment.id).blank?
        @grade = Grade.create(enrollment_id: @enrollment.id, assignment_id: @assignment.id, score: params[:custom_canvas_assignment_points_possible], points_possible: params[:custom_canvas_assignment_points_possible], lis_outcome_service_url: params[:lis_outcome_service_url], lis_result_sourcedid: params[:lis_result_sourcedid])
      #NOTE: no handling right now if the points possible in the assignment are updated by instructor later
        ...
        @grade.update(lis_outcome_service_url: params[:lis_outcome_service_url], lis_result_sourcedid: params[:lis_result_sourcedid])
      end
